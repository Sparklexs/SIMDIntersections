...
// sxs: works in the same way as @getPair(...) in synthetic.h
...
#else
	cout << " Runs an exhaustive benchmark over a ClusterData distribution."
			<< endl;
	cout
			<< " -s followed by comma-separated values specifies intersections functions, chosen from:"
			<< endl;
...
int main(int argc, char **argv) {
	...
	for (float sr = 1.001; sr <= 10000; sr = sr * sqrt(1.9)) {
		vector<pair<vector<uint32_t>, vector<uint32_t>>> data(howmany);
		uint32_t smallsize = static_cast<uint32_t>(round(static_cast<float>(1 << Big) / sr));
		cout << "#generating data...";
		cout.flush();
		for (size_t k = 0; k < howmany; ++k) {
			data[k] = uniform ? getNaivePair(udg , smallsize, 1U << MaxBit, sr, intersectionratio)
			: getNaivePair(cdg , smallsize, 1U << MaxBit, sr, intersectionratio);
		}
		cout << "ok." << endl;
		cout << sr << "\t";
		float aratio = 0.0f;
		for (string intername : myschemes) {
			...
			size_t volume = 0;
#ifdef LIKWID_MARKERS
			snprintf(currentMarker, sizeof(currentMarker), "%s %.2f", intername.c_str(), sr);
