...
void printusage() {
	...
	cout
			...
			<< endl;
	cout
			<< " The -S flag allows you to specific the log. of"
					" the minimal array size (default 10)."
			<< endl;
}
...
int main(int argc, char **argv) {
	size_t howmany = 0;
	size_t loop = 10;
	uint32_t logMinLength = 10; // log of minimal array size
	string name;
	...
	while ((c = getopt(argc, argv, "ns:m:M:S:l:r:h")) != -1)
		...
		case 'h':
			printusage();
			return 0;
		case 'S': //log of minimal array size
			logMinLength = atoi(optarg);
			break;
		case 'M': // largest bit-length of element
			MaxBit = atoi(optarg);
			if (MaxBit < 1) {
				printusage();
				return -1;
			...
			break;
		case 'm': // how many pairs of <set1,set2> to be intersected?
			howmany = atoi(optarg);
			if (howmany < 1) {
				printusage();
				return -1;
	...
	for (float ir : intersectionsratios) {
		...
		for (uint32_t sr : sizeratios) {
			...
			for (size_t k = 0; k < howmany; ++k)
				data[k] = getPair(cdg, minlength, 1U << MaxBit,
						static_cast<float>(sr), ir);
			size_t volume = 0;
			timer.reset();
			for (size_t L = 0; L < loop; ++L) {

				for (auto setPair : data) {
					volume += (setPair.first).size();
					volume += (setPair.second).size();
					bogus += myscheme(&(setPair.first)[0], (setPair.first).size(),
							&(setPair.second)[0], (setPair.second).size(), &buffer[0]);
				}
